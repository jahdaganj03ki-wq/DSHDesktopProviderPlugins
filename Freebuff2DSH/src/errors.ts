...
export function classifyFreebuffError(input: {
  ...
  let klass: FreebuffErrorOutcome['class'] = 'unknown'
  if (['quota_exhausted', 'quota_exceeded', 'spend_limited'].includes(code)) klass = 'quota_exhausted'
  else if (code === 'rate_limited') klass = 'rate_limit'
  else if (['ip_capped', 'ip_cap'].includes(code)) klass = 'ip_cap'
  else if (['banned', 'account_banned'].includes(code)) klass = 'banned'
  else if (['country_blocked', 'geo_blocked'].includes(code)) klass = 'country_blocked'
  else if (['session_superseded', 'session_conflict'].includes(code)) klass = 'session_conflict'
  else if (['model_locked', 'model_unavailable', 'waiting_room_required'].includes(code)) klass = 'model_unavailable'
  else if (status === 401) klass = 'auth'
  else if (status === 403) klass = 'permission'
  else if (status === 400) klass = 'invalid_request'
  else if (status === 408) klass = 'timeout'
  else if (status >= 500) klass = 'server'
  else if (status === 429) klass = 'rate_limit'
  else if (status === 0) klass = 'network'
...
export function maySwitchAccount(outcome: FreebuffErrorOutcome, request: { stream: boolean; sideEffect: string }, policy: { allowStreamingFailover: boolean; allowNonIdempotentFailover: boolean }): boolean {
  if (outcome.bytesSent || outcome.sideEffectAccepted) return false
  if (outcome.class !== 'quota_exhausted') return false
  ...
  if (request.sideEffect !== 'none' && !policy.allowNonIdempotentFailover) return false
  return true
}
