...
function asSession(value, model) {
    const instanceId = asString(value.instanceId);
    if (!instanceId)
        throw new Error('Freebuff session response has no instanceId');
...
export class FreebuffProvider {
    ...
    constructor(accounts, policy, transport = new FreebuffTransport(), persist) {
    ...
    async *stream(request) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            if (!account)
                throw new Error('No eligible Freebuff account remains');
    ...
    transform(request, instanceId, runId) {
    ...
    async ensureSession(account, model) {
        ...
        if (current.status === 'active' && asString(current.model) === model && asString(current.instanceId)) {
            const session = asSession(current, model);
        ...
        const session = asSession(admitted, model);
        this.sessions.set(key, session);
        return session;
    }
    async ensureRun(account, model) {
        const key = `${account.id}:${model}`;
        ...
        if (!runId)
            throw new Error('Freebuff run response has no runId');
