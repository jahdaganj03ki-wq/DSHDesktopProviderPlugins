...
function keyFromConfig(config) {
    ...
    if (!encoded)
        throw new Error('DSH_FREEBUFF_VAULT_KEY is required; set a 32-byte base64url key before adding accounts');
    const key = Buffer.from(encoded, 'base64url');
    if (key.length !== 32)
        throw new Error('DSH_FREEBUFF_VAULT_KEY must decode to exactly 32 bytes');
    return key;
}
export function apply(ctx, config = {}) {
    ...
    if (typeof ctx.inject === 'function')
        ctx.inject(['webServer'], (webCtx) => registerFreebuffRoutes(webCtx, vault));
}
