...
function messageBody(options) {
    ...
    return {
        messages,
        ...(Array.isArray(options.tools) && options.tools.length ? { tools: options.tools.map((tool) => ({ type: 'function', function: { name: tool.name, description: tool.description, parameters: tool.parameters } })) } : {}),
        ...(options.maxTokens !== undefined ? { max_tokens: options.maxTokens } : {}),
    };
}
async function* sseChunks(source, signal) {
    ...
    for await (const bytes of source) {
        ...
        for (const line of lines) {
            ...
            if (event.usage)
                yield { type: 'usage', usage: { input: event.usage.prompt_tokens ?? 0, output: event.usage.completion_tokens ?? 0, total: event.usage.total_tokens ?? 0 } };
        }
    }
    if (started)
...
export class FreebuffLlmAdapter extends LlmAdapter {
    ...
    providerInfo(provider) { return { id: provider, name: 'Freebuff' }; }
    listModels(_provider) { return Promise.resolve(this.provider.models().map(id => ({ provider: 'freebuff', id, name: id, inputModalities: ['text'] }))); }
    resolveModel(provider, model, _signal) { return Promise.resolve({ provider, id: model, name: model, inputModalities: ['text'], context: { contextWindow: 1_048_576 } }); }
    async prepareCall(provider, model, signal) { return { model: await this.resolveModel(provider, model), stream: (options) => this.stream({ ...options, provider, model, signal }) }; }
...
export function createFreebuffLlmAdapter(provider) { return new FreebuffLlmAdapter(provider); }
